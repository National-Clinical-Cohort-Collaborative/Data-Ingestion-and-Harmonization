create or replace PROCEDURE BUILD_P2O_CODE_XWALK AS
BEGIN

    INSERT INTO p2o_code_xwalk_standard ( CDM_TBL, src_code, src_code_type, source_code, source_code_concept_id, source_code_description,
    ...
    select 'Diagnosis' as cdm_tbl, x.src_code as src_code, x.src_code_type as src_code_type,
        ...
        FROM cte_vocab_map
        right outer join native_pcorNet51_cdm.diagnosis d
        on source_code in (d.dx ) -- d.dx code -- in('U07.1')
       ...
       --EXCEPTION
        --WHEN OTHERS THEN
        --DBMS_OUTPUT.ENABLE (buffer_size IN INTEGER DEFAULT 200000);
