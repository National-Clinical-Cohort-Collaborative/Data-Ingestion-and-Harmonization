...
*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_DM_OMOP_DOSE_ERA 
(
...
SELECT distinct 
  ...
  'DOSE_ERA' AS DOMAIN_NAME,
  DOSE_ERA_ID AS SOURCE_ID, 
  SYSDATE AS CREATE_DATE,
  null as target_domain_id, 
  null as target_concept_id
FROM "NATIVE_OMOP531_CDM"."DOSE_ERA" 
JOIN CDMH_STAGING.PERSON_CLEAN ON DOSE_ERA.PERSON_ID=PERSON_CLEAN.PERSON_ID 
                          AND PERSON_CLEAN.DATA_PARTNER_ID=DATAPARTNERID 
LEFT JOIN CDMH_STAGING.N3CDS_DOMAIN_MAP mp on DOSE_ERA.DOSE_ERA_ID=Mp.Source_Id 
                          AND mp.DOMAIN_NAME='DOSE_ERA' 
