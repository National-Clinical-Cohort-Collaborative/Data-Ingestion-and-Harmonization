...
Stored Procedure : SP_DM_OMOP_OBSERVATION

Description : insert to N3CDS_DOMAIN_MAP

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_DM_OMOP_OBSERVATION 
(
...
FROM "NATIVE_OMOP531_CDM"."OBSERVATION"  
JOIN CDMH_STAGING.person_clean ON OBSERVATION.PERSON_ID=person_clean.PERSON_ID 
						AND PERSON_CLEAN.DATA_PARTNER_ID=DATAPARTNERID   
LEFT JOIN CDMH_STAGING.N3CDS_DOMAIN_MAP mp on OBSERVATION.OBSERVATION_ID=Mp.Source_Id 
						AND mp.DOMAIN_NAME='OBSERVATION' AND mp.DATA_PARTNER_ID=DATAPARTNERID  
