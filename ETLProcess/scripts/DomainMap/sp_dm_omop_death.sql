...
Stored Procedure : SP_DM_OMOP_DEATH

Description : insert to N3CDS_DOMAIN_MAP

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_DM_OMOP_DEATH 
(
...
DEATH.PERSON_ID AS SOURCE_ID, 
SYSDATE AS CREATE_DATE,
'PERSON' AS DOMAIN_NAME, 
NULL AS TARGET_DOMAIN_ID, 
NULL AS TARGET_CONCEPT_ID 
FROM "NATIVE_OMOP531_CDM"."DEATH"  
JOIN CDMH_STAGING.person_clean ON DEATH.PERSON_ID=person_clean.PERSON_ID 
                                AND PERSON_CLEAN.DATA_PARTNER_ID=DATAPARTNERID    
LEFT JOIN CDMH_STAGING.N3CDS_DOMAIN_MAP mp on DEATH.PERSON_ID=Mp.Source_Id 
                                AND mp.DOMAIN_NAME='PERSON' 
                                AND mp.DATA_PARTNER_ID=DATAPARTNERID  
WHERE mp.N3CDS_DOMAIN_MAP_ID IS NULL AND 1=2
;
