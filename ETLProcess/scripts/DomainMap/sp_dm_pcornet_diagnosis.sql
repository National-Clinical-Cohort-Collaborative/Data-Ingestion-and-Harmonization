...
Stored Procedure : SP_DM_PCORNET_DIAGNOSIS

Description : insert to N3CDS_DOMAIN_MAP

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_DM_PCORNET_DIAGNOSIS 
(
...
'DIAGNOSIS' AS DOMAIN_NAME,
DIAGNOSISID AS SOURCE_ID, 
SYSDATE AS CREATE_DATE,
x.target_domain_id, 
x.target_concept_id
FROM "NATIVE_PCORNET51_CDM"."DIAGNOSIS"  
JOIN CDMH_STAGING.PERSON_CLEAN ON DIAGNOSIS.PATID=PERSON_CLEAN.PERSON_ID AND PERSON_CLEAN.DATA_PARTNER_ID=DATAPARTNERID  
JOIN CDMH_STAGING.P2o_Code_Xwalk_Standard x ON x.src_code=DIAGNOSIS.DX AND X.Cdm_Tbl='DIAGNOSIS'  
LEFT JOIN CDMH_STAGING.N3CDS_DOMAIN_MAP mp on DIAGNOSIS.DIAGNOSISID=Mp.Source_Id AND mp.DOMAIN_NAME='DIAGNOSIS' 
...
;
TYPE l_val_cur IS TABLE OF Diagnosis_Cursor%ROWTYPE;
...
open Diagnosis_Cursor;
  LOOP
    FETCH Diagnosis_Cursor bulk collect into values_rec limit 10000;
