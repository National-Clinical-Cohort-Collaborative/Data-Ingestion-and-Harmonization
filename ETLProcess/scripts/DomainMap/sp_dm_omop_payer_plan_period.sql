...
Stored Procedure : SP_DM_OMOP_PAYER_PLAN_PERIOD

Description : insert to N3CDS_DOMAIN_MAP

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_DM_OMOP_PAYER_PLAN_PERIOD 
(
...
'PAYER_PLAN_PERIOD' AS DOMAIN_NAME,
PAYER_PLAN_PERIOD_ID AS SOURCE_ID, 
SYSDATE AS CREATE_DATE,
null as target_domain_id, 
null as target_concept_id
FROM "NATIVE_OMOP531_CDM"."PAYER_PLAN_PERIOD"  
JOIN CDMH_STAGING.person_clean ON PAYER_PLAN_PERIOD.PERSON_ID=person_clean.PERSON_ID 
			AND PERSON_CLEAN.DATA_PARTNER_ID=DATAPARTNERID   
LEFT JOIN CDMH_STAGING.N3CDS_DOMAIN_MAP mp on PAYER_PLAN_PERIOD.PAYER_PLAN_PERIOD_ID=Mp.Source_Id 
			AND mp.DOMAIN_NAME='PAYER_PLAN_PERIOD' 
...
END SP_DM_OMOP_PAYER_PLAN_PERIOD;
