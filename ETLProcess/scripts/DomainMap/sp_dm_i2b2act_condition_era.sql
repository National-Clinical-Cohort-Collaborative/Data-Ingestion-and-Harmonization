...
Stored Procedure : SP_DM_OMOP_CONDITION_ERA

Description : insert to N3CDS_DOMAIN_MAP

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_DM_OMOP_CONDITION_ERA 
(
...
SELECT distinct 
      ...
      'CONDITION_ERA' AS DOMAIN_NAME,
      CONDITION_ERA_ID AS SOURCE_ID, 
      SYSDATE AS CREATE_DATE,
      null as target_domain_id, 
      null as target_concept_id
FROM "NATIVE_OMOP531_CDM"."CONDITION_ERA"   
JOIN CDMH_STAGING.PERSON_CLEAN ON CONDITION_ERA.PERSON_ID=PERSON_CLEAN.PERSON_ID 
                            AND PERSON_CLEAN.DATA_PARTNER_ID=DATAPARTNERID   
LEFT JOIN CDMH_STAGING.N3CDS_DOMAIN_MAP mp on CONDITION_ERA.CONDITION_ERA_ID=Mp.Source_Id 
                            AND mp.DOMAIN_NAME='CONDITION_ERA' 
