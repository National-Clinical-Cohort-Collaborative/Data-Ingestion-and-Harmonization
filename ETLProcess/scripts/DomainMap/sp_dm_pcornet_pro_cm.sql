...
Stored Procedure : SP_DM_PCORNET_PRO_CM

Description : insert to N3CDS_DOMAIN_MAP

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_DM_PCORNET_PRO_CM 
(
...
**************************************************************/
CURSOR PCM_Cursor IS
SELECT distinct 
DATAPARTNERID AS DATA_PARTNER_ID,
'PRO_CM' AS DOMAIN_NAME,
PRO_CM_ID AS SOURCE_ID, 
SYSDATE AS CREATE_DATE,
null as target_domain_id, 
null as target_concept_id 
FROM "NATIVE_PCORNET51_CDM"."PRO_CM" 
JOIN CDMH_STAGING.PERSON_CLEAN ON PRO_CM.PATID=PERSON_CLEAN.PERSON_ID 
                                AND PERSON_CLEAN.DATA_PARTNER_ID=DATAPARTNERID    
LEFT JOIN CDMH_STAGING.N3CDS_DOMAIN_MAP mp on PRO_CM.PRO_CM_ID=Mp.Source_Id AND mp.DOMAIN_NAME='PRO_CM'  
                                AND mp.DATA_PARTNER_ID=DATAPARTNERID     
WHERE mp.N3CDS_DOMAIN_MAP_ID IS NULL
;
TYPE l_val_cur IS TABLE OF PCM_Cursor%ROWTYPE;
...
open PCM_Cursor;
  LOOP
    FETCH PCM_Cursor bulk collect into values_rec limit 10000;
