...
Stored Procedure : SP_DM_OMOP_MEASUREMENT

Description : insert to N3CDS_DOMAIN_MAP

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_DM_OMOP_MEASUREMENT 
(
...
FROM "NATIVE_OMOP531_CDM"."MEASUREMENT"  
JOIN CDMH_STAGING.person_clean ON MEASUREMENT.PERSON_ID=person_clean.PERSON_ID 
                          AND PERSON_CLEAN.DATA_PARTNER_ID=DATAPARTNERID   
LEFT JOIN CDMH_STAGING.N3CDS_DOMAIN_MAP mp on MEASUREMENT.MEASUREMENT_ID=Mp.Source_Id 
                          AND mp.DOMAIN_NAME='MEASUREMENT' 
