...
*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_DM_PCORNET_PROCEDURES 
(
...
'PROCEDURES' AS DOMAIN_NAME,
PROCEDURESID AS SOURCE_ID, 
SYSDATE AS CREATE_DATE,
x.target_domain_id, 
x.target_concept_id 
FROM "NATIVE_PCORNET51_CDM"."PROCEDURES"  
JOIN CDMH_STAGING.PERSON_CLEAN ON PROCEDURES.PATID=PERSON_CLEAN.PERSON_ID 
                                      AND PERSON_CLEAN.DATA_PARTNER_ID=DATAPARTNERID   
JOIN CDMH_STAGING.P2o_Code_Xwalk_Standard x ON x.src_code=PROCEDURES.PX 
                                      AND X.Cdm_Tbl='PROCEDURES'  
LEFT JOIN CDMH_STAGING.N3CDS_DOMAIN_MAP mp on PROCEDURES.PROCEDURESID=Mp.Source_Id 
