...
Stored Procedure : SP_DM_PCORNET_IMMUNIZATION

Description : insert to N3CDS_DOMAIN_MAP

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_DM_PCORNET_IMMUNIZATION 
(
...
**************************************************************/
CURSOR Immun_Cursor IS
SELECT distinct 
DATAPARTNERID AS DATA_PARTNER_ID,
'IMMUNIZATION' AS DOMAIN_NAME,
IMMUNIZATIONID AS SOURCE_ID, 
SYSDATE AS CREATE_DATE,
null as target_domain_id, 
null as target_concept_id
FROM "NATIVE_PCORNET51_CDM"."IMMUNIZATION" 
JOIN CDMH_STAGING.PERSON_CLEAN ON IMMUNIZATION.PATID=PERSON_CLEAN.PERSON_ID 
                              AND PERSON_CLEAN.DATA_PARTNER_ID=DATAPARTNERID    
LEFT JOIN CDMH_STAGING.N3CDS_DOMAIN_MAP mp on IMMUNIZATION.IMMUNIZATIONID=Mp.Source_Id 
                              AND mp.DOMAIN_NAME='IMMUNIZATION'  
                              AND mp.DATA_PARTNER_ID=DATAPARTNERID 
WHERE mp.N3CDS_DOMAIN_MAP_ID IS NULL 
;
TYPE l_val_cur IS TABLE OF Immun_Cursor%ROWTYPE;
...
open Immun_Cursor;
  LOOP
    FETCH Immun_Cursor bulk collect into values_rec limit 10000;
...
END SP_DM_PCORNET_IMMUNIZATION;
