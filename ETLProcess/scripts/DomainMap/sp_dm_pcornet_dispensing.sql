...
Stored Procedure : SP_DM_PCORNET_DISPENSING

Description : insert to N3CDS_DOMAIN_MAP

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_DM_PCORNET_DISPENSING 
(
...
'DISPENSING' AS DOMAIN_NAME,
DISPENSINGID AS SOURCE_ID, 
SYSDATE AS CREATE_DATE,
null as target_domain_id, 
null as target_concept_id  
FROM "NATIVE_PCORNET51_CDM"."DISPENSING" 
JOIN CDMH_STAGING.PERSON_CLEAN ON DISPENSING.PATID=PERSON_CLEAN.PERSON_ID 
                                  AND PERSON_CLEAN.DATA_PARTNER_ID=DATAPARTNERID    
LEFT JOIN CDMH_STAGING.N3CDS_DOMAIN_MAP mp on DISPENSING.DISPENSINGID=Mp.Source_Id 
                                  AND mp.DOMAIN_NAME='DISPENSING'  
