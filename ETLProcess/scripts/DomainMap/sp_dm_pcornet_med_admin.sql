...
*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_DM_PCORNET_MED_ADMIN 
(
...
'MED_ADMIN' AS DOMAIN_NAME,
MEDADMINID AS SOURCE_ID, 
SYSDATE AS CREATE_DATE,
x.target_domain_id, 
x.target_concept_id  
FROM "NATIVE_PCORNET51_CDM"."MED_ADMIN"  
JOIN CDMH_STAGING.PERSON_CLEAN ON MED_ADMIN.PATID=PERSON_CLEAN.PERSON_ID 
                                        AND PERSON_CLEAN.DATA_PARTNER_ID=DATAPARTNERID  
JOIN CDMH_STAGING.P2o_Code_Xwalk_Standard x ON x.src_code=MED_ADMIN.MEDADMIN_CODE 
                                        AND X.Cdm_Tbl='MED_ADMIN'  
LEFT JOIN CDMH_STAGING.N3CDS_DOMAIN_MAP mp on MED_ADMIN.MEDADMINID=Mp.Source_Id AND mp.DOMAIN_NAME='MED_ADMIN'  
