...
(
    ...
    src_vocab_code          VARCHAR2(24 BYTE),
...
*/
  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "CDMH_STAGING"."BUILD_P2O_CODE_XWALK" AS 
BEGIN
 ...
 INSERT INTO CDMH_STAGING.p2o_code_xwalk_standard ( CDM_TBL, src_code, src_code_type, src_vocab_code, 
    ...
    select 'DIAGNOSIS' as cdm_tbl, x.src_code as src_code, x.src_code_type as src_code_type, x.src_vocab_code,
    ...
    union all
    -- procedures
    select 'PROCEDURES' as cdm_tbl, y.src_code, y.src_code_type, src_vocab_code,  
        source_code, source_concept_id, source_code_description, source_vocabulary_id, source_domain_id, 
        target_concept_id, target_concept_name, target_vocabulary_id, target_domain_id, target_concept_class_id
    from
    (
        ...
        SELECT distinct 'PROCEDURES' as CDM_TBL, p.px as src_code, p.px_type src_code_type, -- CH cpt4 or hcpcs, OT, 10
         case when p.px_type = '10' then 'ICD10CM'   
             when p.px_type = 'CH' then 'CPT4' -- or HCPCS 
        ...
        on source_code = p.px   
        where source_vocabulary_id in( 'ICD10CM', 'CPT4') ---DIAGNOSIS and PROCEDURES 35,441
        AND target_standard_concept = 'S' 
    ) y
    union all
     select 'PROCEDURES' as cdm_tbl, y.src_code, y.src_code_type, src_vocab_code,  
    ...
    (
        ...
        SELECT distinct 'PROCEDURES' as CDM_TBL, p.px as src_code, p.px_type src_code_type, -- CH cpt4 or hcpcs, OT, 10
         case when p.px_type = '10' then 'ICD10CM'   
             when p.px_type = 'CH' then 'HCPCS' -- or HCPCS 
             when p.px_type = 'OT' then 'OTHER'
             else 'OT' end as src_vocab_code,
             source_code, source_concept_id, source_code_description, source_vocabulary_id, source_domain_id, 
        target_concept_id, target_concept_name, target_vocabulary_id, target_domain_id, target_concept_class_id ---target_concept_id = omop concept id , target_concept_name = concept name target_domain_id = condition
        ...
        join native_pcorNet51_cdm.PROCEDURES p
        on source_code = p.px   
        where source_vocabulary_id in('HCPCS') ---DIAGNOSIS and PROCEDURES 35,441
        AND target_standard_concept = 'S' 
    ) y
    union all
    select 'CONDITION' as cdm_tbl, c.src_code, c.src_code_type, src_vocab_code,
    source_code, source_concept_id, source_code_description, source_vocabulary_id, source_domain_id, 
        target_concept_id, target_concept_name, target_vocabulary_id, target_domain_id, target_concept_class_id
    from
    (
    ...
    union all
    select 'DEATH_CAUSE' as cdm_tbl, dt.src_code, dt.src_code_type, src_vocab_code,
    source_code, source_concept_id, source_code_description, source_vocabulary_id, source_domain_id, 
        target_concept_id, target_concept_name, target_vocabulary_id, target_domain_id, target_concept_class_id
    from
    (
        ...
        on source_code = dt.death_cause_code
        where source_vocabulary_id in( 'ICD9CM', 'ICD10CM') --50,672/ death cause +2 9 or 10
        AND target_standard_concept = 'S' 
    ) dt
    union all
    --LAB_RESULTS_CM
   select 'LAB_RESULT_CM' as cdm_tbl, lab.src_code, lab.src_code_type, src_vocab_code,
    source_code, source_concept_id, source_code_description, source_vocabulary_id, source_domain_id, 
        target_concept_id, target_concept_name, target_vocabulary_id, target_domain_id, target_concept_class_id
    from
    (
    ...
    union all
     select 'DISPENSING' as cdm_tbl, dp.src_code, dp.src_code_type, src_vocab_code, 
     source_code, source_concept_id, source_code_description, source_vocabulary_id, source_domain_id, 
        target_concept_id, target_concept_name, target_vocabulary_id, target_domain_id, target_concept_class_id
    from
    (
    ...
    --med_admin
     ...
     select 'MED_ADMIN' as cdm_tbl, ma.src_code, ma.src_code_type, src_vocab_code,
     source_code, source_concept_id, source_code_description, source_vocabulary_id, source_domain_id, 
        target_concept_id, target_concept_name, target_vocabulary_id, target_domain_id, target_concept_class_id    
    from
    (
    ...
    --prescribing
    union all
     select 'PRESCRIBING' as cdm_tbl, pr.src_code, pr.src_code_type, src_vocab_code, 
        source_code, source_concept_id, source_code_description, source_vocabulary_id, source_domain_id, 
        target_concept_id, target_concept_name, target_vocabulary_id, target_domain_id, target_concept_class_id
    from
    (
    ...
    --obsclin_code
    union all
     select 'OBS_CLIN' as cdm_tbl, ob.src_code, ob.src_code_type, src_vocab_code,
     source_code, source_concept_id, source_code_description, source_vocabulary_id, source_domain_id, 
        target_concept_id, target_concept_name, target_vocabulary_id, target_domain_id, target_concept_class_id
    from
    (
