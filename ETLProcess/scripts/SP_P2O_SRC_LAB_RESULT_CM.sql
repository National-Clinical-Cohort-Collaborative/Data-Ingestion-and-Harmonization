...
Description : Stored Procedure to insert PCORnet NATIVE_PCORNET51_CDM.LAB_RESULT_CM into staging table
...
Parameters: DATAPARTNERID IN NUMBER, MANIFESTID IN NUMBER, RECORDCOUNT OUT NUMBER
     ...
     0.2     6/26/2020  SHONG           Added logic to insert into CDMH_STAGING.ST_OMOP53_OBSERVATION
...
BEGIN
    INSERT INTO CDMH_STAGING.ST_OMOP53_MEASUREMENT (--for lab_result_cm 
        ...
        LEFT JOIN CDMH_STAGING.p2o_code_xwalk_standard xw on lab.lab_loinc = xw.src_code  and xw.CDM_TBL = 'LAB_RESULT_CM' AND xw.target_domain_id = 'Measurement' and xw.target_concept_id=mp.target_concept_id
  ...
  COMMIT;
      insert into CDMH_STAGING.ST_OMOP53_OBSERVATION (
        ...
        ,case when lower(trim(result_qual)) = 'positive' then 45884084
            ...
            when lower(trim(result_qual)) = 'elevatied' then 4328749  --ssh add issue number 55 - 6/26/2020
            when lower(trim(result_qual)) = 'un' then 45880480
            else 45877393
        ...
        LEFT JOIN CDMH_STAGING.N3cds_Domain_Map p on p.Source_Id= lab.PATID AND p.Domain_Name='PERSON' AND p.DATA_PARTNER_ID=DATAPARTNERID
        LEFT JOIN CDMH_STAGING.N3cds_Domain_Map e on e.Source_Id= lab.ENCOUNTERID AND e.Domain_Name='ENCOUNTER' and e.target_domain_id ='Visit' AND e.DATA_PARTNER_ID=DATAPARTNERID
        LEFT JOIN CDMH_STAGING.p2o_code_xwalk_standard xw on lab.lab_loinc = xw.src_code  and xw.CDM_TBL = 'LAB_RESULT_CM' AND xw.target_domain_id = 'Observation' 
