...
Stored Procedure : SP_LD_INST2DS_PROVIDER

Description : Load data from instance to DS

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_LD_INST2DS_PROVIDER 
(
...
AS
/********************************************************************************************************
     Name:      SP_LD_INST2DS_PROVIDER
     Purpose:    Loading The N3C_Instance.PROVIDER Table into N3C_DS.PROVIDER
     Source:
     Revisions:
...
FROM N3C_OMOP531_Instance.PROVIDER ;
...
******************************************************/
DELETE FROM N3C_OMOP531_DS.PROVIDER WHERE DATA_PARTNER_ID=DATAPARTNERID;
COMMIT;
/**************************************************************
_  VARIABLES:
...
BEGIN
   FORALL i IN 1..values_rec.COUNT
	   INSERT INTO N3C_OMOP531_DS.PROVIDER VALUES values_rec(i);
        COMMIT;
	END;
