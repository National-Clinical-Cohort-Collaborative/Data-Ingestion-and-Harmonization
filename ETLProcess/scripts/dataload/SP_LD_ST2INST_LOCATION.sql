...
Stored Procedure : SP_LD_ST2INST_LOCATION

Description : Load data from staging to instance

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_LD_ST2INST_LOCATION 
(
...
/********************************************************************************************************
     Name:      SP_LD_ST2INST_LOCATION
     Purpose:    Loading The CDMH_STAGING.ST_OMOP53_LOCATION Table into N3C_Instance.LOCATION
     Source:
     Revisions:
...
FROM CDMH_STAGING.ST_OMOP53_LOCATION
...
BEGIN
   FORALL i IN 1..values_rec.COUNT
	   INSERT INTO N3C_OMOP531_INSTANCE.location VALUES values_rec(i);
        COMMIT;
	END;
