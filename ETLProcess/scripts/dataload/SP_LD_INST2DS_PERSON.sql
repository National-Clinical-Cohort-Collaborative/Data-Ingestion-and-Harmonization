...
Stored Procedure : SP_LD_INST2DS_PERSON

Description : Load data from instance to DS

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_LD_INST2DS_PERSON 
(
...
AS
/********************************************************************************************************
     Name:      SP_LD_INST2DS_PERSON
     Purpose:    Loading The N3C_Instance.PERSON Table into N3C_DS.PERSON
     Source:
     Revisions:
...
FROM N3C_OMOP531_Instance.PERSON ;
TYPE l_val_cur IS TABLE OF Person_Cursor%ROWTYPE;
...
******************************************************/
DELETE FROM N3C_OMOP531_DS.PERSON WHERE DATA_PARTNER_ID=DATAPARTNERID;
COMMIT;
/**************************************************************
_  VARIABLES:
...
open Person_Cursor;
  LOOP
    FETCH Person_Cursor bulk collect into values_rec limit 10000;
    EXIT WHEN values_rec.COUNT=0;
BEGIN
   FORALL i IN 1..values_rec.COUNT
	   INSERT INTO N3C_OMOP531_DS.PERSON VALUES values_rec(i);
        COMMIT;
	END;
