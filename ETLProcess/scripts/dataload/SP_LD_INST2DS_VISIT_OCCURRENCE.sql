...
Stored Procedure : SP_LD_INST2DS_VISIT_OCCURRENCE

Description : Load data from instance to DS

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_LD_INST2DS_VISIT_OCCURRENCE 
(
...
AS
/********************************************************************************************************
     Name:      SP_LD_INST2DS_Visit_occurrence
     Purpose:    Loading The N3C_Instance.Visit_occurrence Table into N3C_DS.Visit_occurrence
     Source:
     Revisions:
...
FROM N3C_OMOP531_Instance.Visit_occurrence ;
TYPE l_val_cur IS TABLE OF Visit_Cursor%ROWTYPE;
...
******************************************************/
DELETE FROM N3C_OMOP531_DS.Visit_occurrence WHERE DATA_PARTNER_ID=DATAPARTNERID;
COMMIT;
/**************************************************************
_  VARIABLES:
...
open Visit_Cursor;
  LOOP
    FETCH Visit_Cursor bulk collect into values_rec limit 10000;
    EXIT WHEN values_rec.COUNT=0;
BEGIN
   FORALL i IN 1..values_rec.COUNT
	   INSERT INTO N3C_OMOP531_DS.Visit_occurrence VALUES values_rec(i);
        COMMIT;
	END;
...
END SP_LD_INST2DS_Visit_occurrence ;
