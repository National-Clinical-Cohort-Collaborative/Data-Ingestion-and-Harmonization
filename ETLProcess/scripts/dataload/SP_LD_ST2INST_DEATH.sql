...
Stored Procedure : SP_LD_ST2INST_DEATH

Description : Load data from staging to instance

*************************************************************/

CREATE PROCEDURE CDMH_STAGING.SP_LD_ST2INST_DEATH 
(
...
AS
/********************************************************************************************************
     Name:      SP_LD_ST2INST_DEATH
     Purpose:    Loading The CDMH_STAGING.DEATH Table into N3C_Instance.DEATH
     Source:
     Revisions:
...
FROM CDMH_STAGING.ST_OMOP53_DEATH 
WHERE DATA_PARTNER_ID=DATAPARTNERID AND MANIFEST_ID=MANIFESTID;
TYPE l_val_cur IS TABLE OF DEATH_Cursor%ROWTYPE;
...
open DEATH_Cursor;
  LOOP
    FETCH DEATH_Cursor bulk collect into values_rec limit 10000;
    EXIT WHEN values_rec.COUNT=0;
BEGIN
   FORALL i IN 1..values_rec.COUNT
	   INSERT INTO N3C_OMOP531_INSTANCE.DEATH VALUES values_rec(i);
        COMMIT;
	END;
