...
Stored Procedure : SP_LD_ST2INST_VISIT_OCCURRENCE

Description : Load data from staging to instance

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_LD_ST2INST_VISIT_OCCURRENCE 
(
...
/********************************************************************************************************
     Name:      SP_LD_ST2INST_VISIT_OCCURRENCE
     Purpose:    Loading The CDMH_STAGING.ST_OMOP53_VISIT_OCCURRENCE Table into N3C_omop531_Instance.VISIT_OCCURRENCE
     Source:
     Revisions:
...
**************************************************************/
CURSOR Visit_occurrence_Cursor IS
SELECT 
VISIT_OCCURRENCE_ID
,PERSON_ID
,VISIT_CONCEPT_ID
...
FROM CDMH_STAGING.ST_OMOP53_Visit_occurrence
WHERE DATA_PARTNER_ID=DATAPARTNERID AND MANIFEST_ID=MANIFESTID;
TYPE l_val_cur IS TABLE OF Visit_occurrence_Cursor%ROWTYPE;
...
******************************************************/
open Visit_occurrence_Cursor;
  LOOP
    FETCH Visit_occurrence_Cursor bulk collect into values_rec limit 10000;
    EXIT WHEN values_rec.COUNT=0;
BEGIN
   FORALL i IN 1..values_rec.COUNT
	   INSERT INTO N3C_OMOP531_INSTANCE.Visit_occurrence VALUES values_rec(i);
        COMMIT;
	END;
...
COMMIT;
Close Visit_occurrence_Cursor;
dbms_output.put_line('Number of records inserted are = '||RECORDCOUNT);


END SP_LD_ST2INST_VISIT_OCCURRENCE;
