...
Stored Procedure : SP_LD_ST2INST_DRUG_ERA

Description : Load data from staging to instance

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_LD_ST2INST_DRUG_ERA 
(
...
AS
/********************************************************************************************************
     Name:      SP_LD_ST2INST_DRUG_ERA
     Purpose:    Loading The CDMH_STAGING.DRUG_ERA Table into N3C_Instance.DRUG_ERA
     Source:
     Revisions:
...
DRUG_EXPOSURE_COUNT,
GAP_DAYS
FROM cdmh_staging.ST_OMOP53_DRUG_ERA 
WHERE DATA_PARTNER_ID=DATAPARTNERID AND MANIFEST_ID=MANIFESTID;
TYPE l_val_cur IS TABLE OF DRUG_ERA_Cursor%ROWTYPE;
...
******************************************************/
open DRUG_ERA_Cursor;
  LOOP
    FETCH DRUG_ERA_Cursor bulk collect into values_rec limit 10000;
    EXIT WHEN values_rec.COUNT=0;
BEGIN
   FORALL i IN 1..values_rec.COUNT
	   INSERT INTO N3C_OMOP531_INSTANCE.DRUG_ERA VALUES values_rec(i);
        COMMIT;
	END;
