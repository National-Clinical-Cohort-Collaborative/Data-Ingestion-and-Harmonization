...
Stored Procedure : SP_LD_ST2INST_DOSE_ERA

Description : Load data from staging to instance

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_LD_ST2INST_DOSE_ERA 
(
...
AS
/********************************************************************************************************
     Name:      SP_LD_ST2INST_DOSE_ERA
     Purpose:    Loading The CDMH_STAGING.DOSE_ERA Table into N3C_Instance.DOSE_ERA
     Source:
     Revisions:
...
DOSE_ERA_START_DATE,
DOSE_ERA_END_DATE
FROM CDMH_STAGING.ST_OMOP53_DOSE_ERA 
WHERE DATA_PARTNER_ID=DATAPARTNERID AND MANIFEST_ID=MANIFESTID;
TYPE l_val_cur IS TABLE OF Dose_era_Cursor%ROWTYPE;
...
******************************************************/
open DOSE_ERA_Cursor;
  LOOP
    FETCH DOSE_ERA_Cursor bulk collect into values_rec limit 10000;
    EXIT WHEN values_rec.COUNT=0;
BEGIN
   FORALL i IN 1..values_rec.COUNT
	   INSERT INTO N3C_OMOP531_INSTANCE.DOSE_ERA VALUES values_rec(i);
        COMMIT;
	END;
