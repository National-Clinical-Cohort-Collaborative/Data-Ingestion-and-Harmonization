...
Stored Procedure : SP_LD_INST2DS_DEATH

Description : Load data from instance to DS

*************************************************************/
CREATE PROCEDURE CDMH_STAGING.SP_LD_INST2DS_DEATH 
(
...
FROM N3C_OMOP531_Instance.DEATH ;
...
******************************************************/
DELETE FROM N3C_OMOP531_DS.Death WHERE DATA_PARTNER_ID=DATAPARTNERID;
COMMIT;
/**************************************************************
_  VARIABLES:
...
BEGIN
   FORALL i IN 1..values_rec.COUNT
	   INSERT INTO N3C_OMOP531_DS.Death VALUES values_rec(i);
        COMMIT;
	END;
