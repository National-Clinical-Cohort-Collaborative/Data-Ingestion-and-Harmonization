...
Parameters: DATAPARTNERID IN NUMBER, MANIFESTID IN NUMBER, RECORDCOUNT OUT NUMBER
     ...
     0.2     6/25/2020   SHONG      Added logic to insert into CDMH_STAGING.ST_OMOP53_OBSERVATION
     0.3     6/26/2020   SHONG      Added logic to insert into CDMH_STAGING.ST_OMOP53_MEASUREMENT and CDMH_STAGING.ST_OMOP53_DRUG_EXPOSURE
...
insert into CDMH_STAGING.ST_OMOP53_OBSERVATION (
    ...
    LEFT JOIN cdmh_staging.N3cds_Domain_Map p on p.Source_Id=c.PATID AND p.Domain_Name='PERSON' AND p.DATA_PARTNER_ID=DATAPARTNERID
    LEFT JOIN cdmh_staging.N3cds_Domain_Map e on e.Source_Id=c.ENCOUNTERID AND e.Domain_Name='ENCOUNTER' and e.target_domain_id ='Visit' AND e.DATA_PARTNER_ID=DATAPARTNERID
    LEFT JOIN CDMH_STAGING.p2o_code_xwalk_standard xw on c.condition = xw.src_code and xw.CDM_TBL='CONDITION' AND xw.target_domain_id = 'Observation'  
...
COMMIT;
   ...
   --insert condition to procedure
    ...
    LEFT JOIN cdmh_staging.N3cds_Domain_Map p on p.Source_Id=c.PATID AND p.Domain_Name='PERSON' AND p.DATA_PARTNER_ID= DATAPARTNERID
    LEFT JOIN cdmh_staging.N3cds_Domain_Map e on e.Source_Id=c.ENCOUNTERID AND e.Domain_Name='ENCOUNTER' and e.target_domain_id ='Visit' AND e.DATA_PARTNER_ID= DATAPARTNERID
    LEFT JOIN CDMH_STAGING.p2o_code_xwalk_standard xw on c.condition = xw.src_code and xw.CDM_TBL='CONDITION' AND xw.target_domain_id = 'Procedure'
                                                       and xw.target_concept_id=mp.target_concept_id
                                                       and Xw.Src_Code_Type=c.condition_type
                                                       ;
...
--        LEFT JOIN CDMH_STAGING.N3cds_Domain_Map prv on prv.Source_Id=e.source_id.providerid AND prv.Domain_Name='PROVIDER' AND prv.DATA_PARTNER_ID=2000
        LEFT JOIN CDMH_STAGING.P2O_CODE_XWALK_STANDARD xw on c.condition = xw.src_code and xw.CDM_TBL='CONDITION' AND xw.target_domain_id = 'Measurement'
                                                            and xw.target_concept_id=mp.target_concept_id 
                                                            and Xw.Src_Code_Type=c.condition_type
...
SELECT
    ...
    c.RESOLVE_DATE as drug_exposure_end_date,
    ...
    'PCORNET_CONDTION' as DOMAIN_SOURCE
  FROM NATIVE_PCORNET51_CDM.CONDITION c
  ...
  LEFT JOIN CDMH_STAGING.N3cds_Domain_Map p on p.Source_Id=c.PATID AND p.Domain_Name='PERSON' AND p.DATA_PARTNER_ID=DATAPARTNERID
  LEFT JOIN CDMH_STAGING.N3cds_Domain_Map e on e.Source_Id=c.ENCOUNTERID AND e.Domain_Name='ENCOUNTER' and e.target_domain_id ='Visit' AND e.DATA_PARTNER_ID=DATAPARTNERID
  LEFT JOIN CDMH_STAGING.p2o_code_xwalk_standard xw on c.condition = xw.src_code and xw.CDM_TBL='CONDITION' 
