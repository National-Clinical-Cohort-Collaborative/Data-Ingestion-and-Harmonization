CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 605/transform/04 - mapping/condition_era` AS
...
(
	SELECT
		...
		, co.site_person_id as person_id -- refer to site_person_id as person_id for intermediate steps, then rename in final SELECT statement
		, co.condition_concept_id
		, co.condition_start_date
		, COALESCE(NULLIF(co.condition_end_date,NULL), date_add(condition_start_date, 1)) AS condition_end_date
		, data_partner_id
		, payload
	FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 605/transform/04 - mapping/condition_occurrence` co
