...
output_checks = [
    Check(E.primary_key('condition_occurrence_id'), 'Valid cleaned primary key', on_error='WARN')
]
input_checks = [
...
@transform(
    processed=Output(
        "/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 605/transform/02 - clean/condition_occurrence",
        checks=output_checks
    ),
    my_input=Input(
        "/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 605/transform/01 - parsed/condition_occurrence",
        checks=input_checks
    ),
