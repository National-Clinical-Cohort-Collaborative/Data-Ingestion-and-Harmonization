...
output_checks = [
    Check(E.primary_key('observation_period_id'), 'Valid cleaned primary key', on_error='WARN')
]
input_checks = [
...
@transform(
    processed=Output(
        "/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 605/transform/02 - clean/observation_period",
        checks=output_checks
    ),
    my_input=Input(
        "/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 605/transform/01 - parsed/observation_period",
        checks=input_checks
    ),
