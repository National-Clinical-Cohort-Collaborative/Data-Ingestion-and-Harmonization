...
output_checks = [
    Check(E.primary_key('care_site_id'), 'Valid cleaned primary key', on_error='WARN')
]
input_checks = [
    Check(E.count().gt(0), 'Required table is not empty', on_error='WARN'),
...
@transform(
    processed=Output(
        "/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 605/transform/02 - clean/care_site",
        checks=output_checks
    ),
    my_input=Input(
        "/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 605/transform/01 - parsed/care_site",
        checks=input_checks
    ),
