...
output_checks = [
    Check(E.primary_key('procedure_occurrence_id'), 'Valid cleaned primary key', on_error='WARN')
]
input_checks = [
    Check(E.count().gt(0), 'Required table is not empty', on_error='FAIL'),
...
@transform(
    processed=Output(
        "/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 605/transform/02 - clean/procedure_occurrence",
        checks=output_checks
    ),
    my_input=Input(
        "/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 605/transform/01 - parsed/procedure_occurrence",
        checks=input_checks
    ),
