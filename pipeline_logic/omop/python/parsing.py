...
from myproject.schemas import complete_domain_schema_dict, schema_dict_to_struct
from myproject.site_specific_utils import get_site_dialect_params
        ...
        with tempfile.NamedTemporaryFile() as t:
            ...
            with open(t.name, newline="", encoding="utf8", errors='ignore') as f:
                with error_df.filesystem().open('error_rows', 'w', newline='') as writeback:
                    ...
                    header = next(r)
                    header = [x.strip("\r").strip("\ufeff") for x in header]
                    MyRow = Row(*header)
