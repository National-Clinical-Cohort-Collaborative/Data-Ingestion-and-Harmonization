...
def apply_schema(df, schema_dict):
        ...
        if col_name in input_cols:
            if col_name in primary_key_cols:
                ...
                df = df.withColumn(
                    col_name,
                    F.when(
                        df[col_name].rlike("^[\d]{2}-[A-Za-z]{3}-[\d]{2} [\d]{2}\.[\d]{2}\.[\d]{2}\.[\d]{9} [AM]|[PM]$"),
                        F.from_unixtime(F.unix_timestamp(df[col_name], format='dd-MMM-yy hh.mm.ss.SSSSSSSSS a'))
    ...
    df = df.select(list(schema_dict.keys()) + ['PAYLOAD'])
