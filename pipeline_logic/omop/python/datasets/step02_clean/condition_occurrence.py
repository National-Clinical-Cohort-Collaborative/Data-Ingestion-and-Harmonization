...
input_checks = [
    Check(E.count().gt(0), 'Required OMOP table is not empty', on_error='FAIL'),
...
@transform(
    processed=Output(
        "/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: OMOP/Site 25/transform/02 - clean/condition_occurrence",
        checks=output_checks
    ),
    my_input=Input(
        "/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: OMOP/Site 25/transform/01 - parsed/condition_occurrence",
        checks=input_checks
    ),
