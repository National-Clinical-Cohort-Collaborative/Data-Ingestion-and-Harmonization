CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 77/transform/04 - domain mapping/drug_era` AS
...
(-- Normalize DRUG_EXPOSURE_END_DATE to either the existing drug exposure end date, or add days supply, or add 1 day to the start date
	SELECT
		...
		, d.site_patient_id as person_id -- refer to site_patient_id as person_id for intermediate steps, then rename in final SELECT statement
		...
		, data_partner_id
		, payload
	FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 77/transform/04 - domain mapping/drug_exposure` d
		JOIN `/UNITE/OMOP Vocabularies/concept_ancestor` ca ON ca.descendant_concept_id = d.drug_concept_id
		JOIN `/UNITE/OMOP Vocabularies/concept` c ON ca.ancestor_concept_id = c.concept_id
