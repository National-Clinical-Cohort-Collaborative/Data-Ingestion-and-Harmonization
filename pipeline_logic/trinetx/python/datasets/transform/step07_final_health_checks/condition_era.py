...
all_checks = [
    Check(E.primary_key('condition_era_id'), 'Valid primary key', on_error='FAIL'),
    ...
    Check(E.col('condition_era_start_date').non_null(), 'condition_era_start_date column contains null', on_error='WARN'),
    Check(E.col('condition_era_end_date').non_null(), 'condition_era_end_date column contains null', on_error='WARN')
]


@transform_df(
    Output('/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 77/final/condition_era'),
    my_input=Input(
        '/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 77/transform/06 - id generation/condition_era',
        checks=all_checks
    ),
)
