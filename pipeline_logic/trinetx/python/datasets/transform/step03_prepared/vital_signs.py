...
@transform(
    processed=Output('/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 77/transform/03 - prepared/vital_signs'),
    my_input=Input('/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 77/transform/02 - clean/vital_signs'),
...
def compute_function(my_input, mapping_table, site_id_df, processed):
    ...
    processed_df = add_coalesced_code_cols(
        processed_df,
        second_mapped_code_system_col='VITAL_CODE_SYSTEM',
        second_mapped_code_col='VITAL_CODE'
    )
