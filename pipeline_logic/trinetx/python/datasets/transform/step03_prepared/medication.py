...
@transform(
    processed=Output('/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 77/transform/03 - prepared/medication'),
    my_input=Input('/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 77/transform/02 - clean/medication'),
...
def compute_function(my_input, mapping_table, site_id_df, processed):
    ...
    processed_df = add_coalesced_code_cols(
        processed_df,
        second_mapped_code_system_col='RX_CODE_SYSTEM',
        second_mapped_code_col='RX_CODE'
    )
