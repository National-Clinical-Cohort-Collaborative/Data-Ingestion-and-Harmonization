...
@transform(
    processed=Output('/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 77/transform/02 - clean/vital_signs'),
    my_input=Input(
        '/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 77/transform/01 - parsed/vital_signs',
        checks=Check(schema_expectation, 'Dataset from site includes all expected columns', on_error='WARN')
    ),
)
def compute_function(my_input, processed):
    ...
    processed_df = processed_df.withColumn("MEASURE_DATE", processed_df["MEASURE_DATE"].cast(T.DateType()))
