CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/04 - domain mapping/observation_period` AS
...
with obs_period AS (
	...
	FROM (
		...
		FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/04 - domain mapping/condition_occurrence`
		GROUP BY site_patid, data_partner_id, payload
		UNION 
            SELECT 
				  site_patid
				, data_partner_id
            ...
            FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/04 - domain mapping/drug_exposure`
		    GROUP BY site_patid, data_partner_id, payload
		UNION 
            SELECT 
				  site_patid
            ...
            FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/04 - domain mapping/procedure_occurrence`
	        GROUP BY site_patid, data_partner_id, payload
	    UNION
            SELECT 
				  site_patid
            ...
            FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/04 - domain mapping/observation`
		    GROUP BY site_patid, data_partner_id, payload
		UNION 
            SELECT 
				  site_patid
            ...
            FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/04 - domain mapping/measurement`
		    GROUP BY site_patid, data_partner_id, payload
		UNION 
		    SELECT 
				  site_patid
			...
			FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/04 - domain mapping/device_exposure`
		    GROUP BY site_patid, data_partner_id, payload
		UNION 
			SELECT 
				  site_patid
			...
			FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/04 - domain mapping/visit_occurrence`
			GROUP BY site_patid, data_partner_id, payload
		UNION 
			SELECT
				  site_patid
			...
			FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/04 - domain mapping/death`
			GROUP BY site_patid, data_partner_id, payload
	) t
	GROUP BY t.site_patid, t.data_partner_id, t.payload
)
