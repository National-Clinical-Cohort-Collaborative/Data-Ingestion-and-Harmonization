CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/04 - domain mapping/observation` AS
...
-- PAT_PREF_LANGUAGE_SPOKEN
    ...
    FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/03 - prepared/demographic` demo
        ...
        LEFT JOIN (
            ...
            FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/03 - prepared/encounter` 
            GROUP BY patid
            ) ee 
...
obs_clin as (
    SELECT
        ...
        CAST(null as int) AS provider_id, --for now, still TBD, Governance group question, Davera will follow up
        encounterid as site_encounterid,
        CAST(null as int) AS visit_detail_id,
...
procedures as (
    SELECT
        ...
        payload
    FROM
        `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/03 - prepared/procedures` pr
        JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/p2o_code_xwalk` xw 
...
encounter as (
    SELECT
        ...
        data_partner_id,
        payload
    FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/03 - prepared/encounter` d
...
all_other_tables AS (
    ...
    FROM (
        ...
        FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/03 - prepared/dispensing` d
        INNER JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/p2o_code_xwalk` xw 
