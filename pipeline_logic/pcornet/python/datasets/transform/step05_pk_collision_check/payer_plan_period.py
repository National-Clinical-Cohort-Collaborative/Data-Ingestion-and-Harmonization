...
@transform(
    lookup_df=Output(
        "/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/05 - pkey collision lookup tables/payer_plan_period",
        checks=Check(E.col('collision_bits').lt(4), 'Fewer than 3 collisions for each 51-bit id', on_error='FAIL')
    ),
    omop_domain=Input(
        "/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/04 - domain mapping/payer_plan_period",
        checks=Check(E.primary_key('hashed_id'), 'hashed_id is unique', on_error='FAIL')
    ),
)
def my_compute_function(omop_domain, lookup_df, ctx):

    pk_col = "payer_plan_period_id_51_bit"
