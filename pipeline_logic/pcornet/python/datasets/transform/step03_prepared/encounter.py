...
@transform(
    processed=Output('/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/03 - prepared/encounter'),
    my_input=Input('/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/02 - clean/encounter'),
...
def compute_function(my_input, site_id_df, processed):
    ...
    processed_df = create_datetime_col(processed_df, "admit_date", "admit_time", "ADMIT_DATETIME")
    processed_df = create_datetime_col(processed_df, "discharge_date", "discharge_time", "DISCHARGE_DATETIME")
