...
@transform(
    processed=Output(
        '/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/02 - clean/encounter',
        checks=[
            Check(E.primary_key('encounterid'), 'Valid cleaned primary key', on_error='WARN')
        ]
    ),
    my_input=Input(
        '/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/01 - parsed/encounter',
        checks=[
            ...
            Check(E.any(E.primary_key('ENCOUNTERID'), E.primary_key('encounterid')), 'Valid parsed primary key', on_error='WARN')
        ]
    ),
)
