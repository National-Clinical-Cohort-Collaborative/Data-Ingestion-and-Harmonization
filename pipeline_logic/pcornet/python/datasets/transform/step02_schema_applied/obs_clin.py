...
table_schema_pcornet_v5 = complete_domain_schema_dict["obs_clin_5.0"]
# Get required PCORnet columns -- handle PCORnet 5.0 and 6.0 schemas
required_schema = required_domain_schema_dict[domain]
required_schema_pcornet_v5 = required_domain_schema_dict["obs_clin_5.0"]
...
@transform(
    processed=Output(
        '/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/02 - clean/obs_clin',
        checks=[
            Check(E.primary_key('obsclinid'), 'Valid cleaned primary key', on_error='WARN')
        ]
    ),
    my_input=Input(
        '/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/01 - parsed/obs_clin',
        checks=[
            Check(schema_expectation, 'Dataset from site includes all expected columns', on_error='WARN'),
            Check(E.any(E.primary_key('OBSCLINID'), E.primary_key('obsclinid')), 'Valid parsed primary key', on_error='WARN')
        ]
    ),
)
