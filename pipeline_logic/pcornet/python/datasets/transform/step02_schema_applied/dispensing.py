...
@transform(
    processed=Output(
        '/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/02 - clean/dispensing',
        checks=[
            Check(E.primary_key('dispensingid'), 'Valid cleaned primary key', on_error='WARN')
        ]
    ),
    my_input=Input(
        '/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/01 - parsed/dispensing',
        checks=[
            Check(schema_expectation, 'Dataset from site includes all expected columns', on_error='WARN'),
            Check(E.any(E.primary_key('DISPENSINGID'), E.primary_key('dispensingid')), 'Valid parsed primary key', on_error='WARN')
        ]
    ),
)
