...
@transform(
    processed=Output(
        '/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/02 - clean/lds_address_history',
        checks=[
            Check(E.primary_key('addressid'), 'Valid cleaned primary key', on_error='WARN')
        ]
    ),
    my_input=Input(
        '/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 793/transform/01 - parsed/lds_address_history',
        checks=[
            Check(schema_expectation, 'Dataset from site includes all expected columns', on_error='WARN'),
            Check(E.any(E.primary_key('ADDRESSID'), E.primary_key('addressid')), 'Valid parsed primary key', on_error='WARN')
        ]
    ),
)
