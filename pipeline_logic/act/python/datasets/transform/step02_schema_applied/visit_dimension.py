...
@transform(
    processed=Output(
        '/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: ACT/Site 411/transform/02 - clean/visit_dimension',
        checks=[
            Check(E.primary_key('patient_num', 'encounter_num'), 'Valid cleaned primary key columns', on_error='WARN')
        ]
    ),
    my_input=Input(
        ...
        checks=[
            ...
            Check(E.any(
                E.primary_key('patient_num', 'encounter_num'),
                E.primary_key('patient_num'.upper(), 'encounter_num'.upper())
