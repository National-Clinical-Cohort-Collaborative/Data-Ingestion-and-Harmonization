CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: ACT/Site 411/transform/04 - domain mapping/measurement`
...
UNION ALL
    ...
    SELECT
        ...
        -----ssh UK: data had null in tval_char and categorical values in the valuelflag_cd 8/1/2020
        CASE
            WHEN upper(o.concept_cd) like 'ACT|LOCAL|LAB%EQUIVOCAL%' or upper(o.concept_cd) like '%UMLS:C4303880%' then 45884087  
            WHEN upper(o.concept_cd) like 'ACT|LOCAL|LAB%NEGATIVE%' or upper(o.concept_cd) like '%UMLS:C1334932%' then 45878583
            WHEN upper(o.concept_cd) like 'ACT|LOCAL|LAB%PENDING%' or upper(o.concept_cd) like '%UMLS:C1611271%' then 1177297 
            WHEN upper(o.concept_cd) like 'ACT|LOCAL|LAB%POSITIVE%' or upper(o.concept_cd) like '%UMLS:C1335447%' then 45884084
    ...
    FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: ACT/Site 411/transform/03 - prepared/observation_fact` o
        ...
        LEFT JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: ACT/Reference Tables/a2o_valueset_mapping_table` tvqual 
            ...
            AND tvqual.CDM_TBL_COLUMN_NAME = 'TVAL_CHAR'
    WHERE 
        upper(o.concept_cd) like '%UMLS:C4303880%' 
            OR upper(o.concept_cd) like '%UMLS:C1334932%'
            OR upper(o.concept_cd) like '%UMLS:C1611271%'
            OR upper(o.concept_cd) like '%UMLS:C1335447%' OR
        upper(o.concept_cd) like '%ACT|LOCAL|LAB%'
...
UNION ALL
    ...
    FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: ACT/Site 411/transform/03 - prepared/observation_fact` o
        ...
        LEFT JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: ACT/Reference Tables/a2o_valueset_mapping_table` tvqual 
            ...
            AND tvqual.CDM_TBL_COLUMN_NAME = 'TVAL_CHAR'
        WHERE
            upper(o.concept_cd) LIKE 'LOINC%NEGATIVE'
            OR upper(o.concept_cd) LIKE 'LOINC%POSITIVE'
            OR upper(o.concept_cd) LIKE 'LOINC%EQUIVOCAL'
            OR upper(o.concept_cd) LIKE 'LOINC%PENDING'
),
