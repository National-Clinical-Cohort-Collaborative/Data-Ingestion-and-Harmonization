CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 777/transform/04 - domain mapping/control_map` AS
    SELECT 
        * 
    ...
    FROM (
        ...
        FROM ( 
            ...
            payload
            FROM 
            ( select distinct case_patid,
                buddy_num,
                control_patid, 
                data_partner_id,
                payload
                from 
                `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 777/transform/03 - prepared/control_map` cm
                WHERE cm.case_patid IS NOT NULL and control_patid IS NOT NULL and buddy_num is not null
            )
        )
    )           