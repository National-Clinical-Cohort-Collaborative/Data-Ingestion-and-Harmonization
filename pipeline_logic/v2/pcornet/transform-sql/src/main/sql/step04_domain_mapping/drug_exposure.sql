CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 777/transform/04 - domain mapping/drug_exposure`
...
with condition as (
    SELECT
        ...
        CAST(null AS long) AS provider_id, --m.medadmin_providerid as provider_id,
...
prescribing as (
    SELECT
        ...
        CASE
            ...
            THEN
                CASE
                    ...
                    ELSE DATE_ADD(COALESCE(rx_start_date, rx_order_date), CAST((rx_days_supply - 1) AS INT))
                END
            ELSE rx_end_date
        ...
        data_partner_id,
        payload
    FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 777/transform/03 - prepared/prescribing` pr
...
immunization as (
    SELECT
        patid AS site_patid,
        CAST(xw.target_concept_id as int) AS drug_concept_id,
        CAST(im.vx_admin_date as date) AS drug_exposure_start_date,
        CAST(null as timestamp) AS drug_exposure_start_datetime,
        CAST(im.vx_admin_date as date) AS drug_exposure_end_date,
        CAST(null as timestamp) AS drug_exposure_end_datetime,
        CAST(null as date) as verbatim_end_date,
        CAST(COALESCE(xw2.TARGET_CONCEPT_ID, 0) as int) AS drug_type_concept_id,
        CAST(null as string) AS stop_reason,
        ...
        CAST(null AS int) AS days_supply,
        CAST(null AS string) AS sig,
        CAST(COALESCE(xw3.TARGET_CONCEPT_ID, 0) as int) AS route_concept_id,
        vx_lot_num AS lot_number,
        CAST(vx_providerid AS int) AS provider_id,
        encounterid as site_encounterid,
        CAST(null as long) AS visit_detail_id,
        CAST(im.vx_code as string) AS drug_source_value,
        CAST(xw.source_concept_id as int) AS drug_source_concept_id,
        CAST(im.vx_route AS string) AS route_source_value,
        CAST(im.vx_dose_unit AS string) AS dose_unit_source_value,
        'IMMUNIZATION' AS domain_source,
        immunizationid as site_pkey,
        data_partner_id,
        payload
    FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 777/transform/03 - prepared/immunization` im
        INNER JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 777/transform/p2o_code_xwalk` xw 
            ON trim(im.vx_code) = trim(xw.src_code)
            AND xw.CDM_TBL = 'IMMUNIZATION' AND xw.target_domain_id = 'Drug' 
            AND upper(trim(xw.src_vocab_code)) = upper(trim(im.mapped_vx_code_type))
        LEFT JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/p2o_valueset_mapping_table` xw2 
            ON upper(trim(im.vx_source)) = upper(trim(xw2.SRC_CODE)) 
            AND xw2.CDM_TBL = 'IMMUNIZATION' AND xw2.CDM_TBL_COLUMN_NAME = 'VX_SOURCE'  
        LEFT JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/p2o_valueset_mapping_table` xw3
            ON upper(trim(im.vx_route)) = upper(trim(xw3.SRC_CODE)) 
            AND xw3.CDM_TBL = 'IMMUNIZATION' AND xw3.CDM_TBL_COLUMN_NAME = 'VX_ROUTE'  
    WHERE im.vx_status = 'CP'                    
