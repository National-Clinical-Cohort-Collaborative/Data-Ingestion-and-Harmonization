CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 777/transform/04 - domain mapping/measurement`
...
lab_result_cm as (
    SELECT
        ...
        --- if the unit source value is null then we should return null instead of 0 for no matching concept
        CASE WHEN ISNULL(lab.result_unit) then NULL
            ELSE CAST(u.TARGET_CONCEPT_ID as int) 
    ...
    INNER JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 777/transform/p2o_code_xwalk` xw 
        ...
        AND trim(lab.lab_loinc) = xw.src_code
    ...
    LEFT JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/p2o_valueset_mapping_table` u 
        ON u.CDM_TBL = 'PRESCRIBING' AND u.CDM_TBL_COLUMN_NAME = 'RX_DOSE_ORDERED_UNIT' 
        AND trim(lab.result_unit) = u.SRC_CODE 
    LEFT JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/p2o_valueset_mapping_table` xw2 
...
obs_clin as (
    SELECT
        ...
        CAST(xwresult.TARGET_CONCEPT_ID as int) AS value_as_concept_id,
    ...
    FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 777/transform/03 - prepared/obs_clin` obs
        ...
        LEFT JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/p2o_valueset_mapping_table` u 
            ON u.CDM_TBL = 'PRESCRIBING' and obs.obsclin_result_unit = u.SRC_CODE
            AND u.CDM_TBL_COLUMN_NAME = 'RX_DOSE_ORDERED_UNIT'
        LEFT JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/p2o_valueset_mapping_table` xwresult
            ON xwresult.CDM_TBL = 'OBS_CLIN'
            AND xwresult.CDM_TBL_COLUMN_NAME = 'OBSCLIN_RESULT_QUAL'
            AND xwresult.TARGET_VOCABULARY_ID = 'LOINC' 
            AND obs.obsclin_type= 'LC' 
            AND upper(trim(obs.obsclin_result_qual)) = upper(trim(xwresult.SRC_CODE))  --Add missing mapping entries (NORMAL, ABNORMAL, OT, NI)
),

-- if there are LC obsgen_type = LC, LOINC coded with numeric value that maps to measurement domain then add them to the  
-- measurement domain with 'OBS_GEN' AS domain_source
obsgen_lc_num as (

    SELECT patid AS site_patid, 
        CAST(xw.target_concept_id as int) AS measurement_concept_id,
        CAST(obg.obsgen_start_date as date) AS measurement_date,
        CAST(obg.OBSGEN_START_DATETIME as timestamp) AS measurement_datetime,
        CAST(obg.obsgen_start_time as string) AS measurement_time,
        32817 AS measurement_type_concept_id,
        CAST(null as int) AS operator_concept_id,
        CAST(obg.obsclin_result_num as float) AS value_as_number,
        CAST(null as int) AS value_as_concept_id,
        CAST(unit.TARGET_CONCEPT_ID as int) AS unit_concept_id,
        CAST(null as float) AS range_low,
        CAST(null as float) AS range_high,
        CAST(null as long) AS provider_id,
        encounterid as site_encounterid,
        CAST(null as long) AS visit_detail_id,
        CAST(obg.obsclin_code as string) AS measurement_source_value,
        CAST(xw.source_concept_id as int) AS measurement_source_concept_id,
        CAST(obs.obsgen_result_unit as string) AS unit_source_value,
        CAST(obs.obsgen_result_num as string) AS value_source_value,
    ...
    INNER JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/Site 777/transform/p2o_code_xwalk` xw 
        ON obg.obsgen_code = xw.src_code AND xw.src_vocab_code = 'LOINC' AND xw.CDM_TBL = 'OBS_GEN' 
        AND xw.target_domain_id = 'Measurement' AND obg.obsgen_type = 'LC' 
    --AND obg.obsgen_result_num is not null ----LC type with numeric result value
    LEFT JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PCORnet/p2o_valueset_mapping_table` unit
        ON u.CDM_TBL = 'PRESCRIBING' AND trim(obg.obsgen_result_unit) = unit.SRC_CODE AND u.CDM_TBL_COLUMN_NAME = 'RX_DOSE_ORDERED_UNIT'
    where obg.obsgen_result_num is not null        
),
