...
def make_transform(domain, pkey):
    ...
    @transform(
        omop_domain=Input(path.transform + "04 - mapping/" + domain, checks=input_check),
        lookup_df=Output(path.transform + "05 - id collision lookup tables/" + domain, checks=output_check),
    )
