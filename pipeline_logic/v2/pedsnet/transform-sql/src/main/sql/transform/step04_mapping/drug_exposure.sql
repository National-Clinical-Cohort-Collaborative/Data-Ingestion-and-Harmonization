CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 777/transform/04 - mapping/drug_exposure` AS
...
with drugexp as (
     SELECT
            ...
            , data_partner_id
            , payload
        FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 777/transform/03 - prepared/drug_exposure` d
        WHERE drug_exposure_id IS NOT NULL
        -- Retain all records from the source table (i.e. don't filter on domain_id) since we aren't mapping any records from 
        -- the source Drug Exposure table into other domains. 
    ), 
