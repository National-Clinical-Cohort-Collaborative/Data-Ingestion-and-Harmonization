CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 777/transform/04 - mapping/measurement` AS
    ...
    with measurement as (
        SELECT
            ...
            , data_partner_id
            , payload
        FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 777/transform/03 - prepared/measurement` m
        WHERE measurement_id IS NOT NULL
        -- Retain all records from the source table, unless we're already mapping them to another domain
        AND (m.target_domain_id IS NULL or m.target_domain_id not in ('Observation'))
    ),
    ...
    observation as (
        SELECT
            ...
            , person_id as site_person_id
            , o.target_concept_id as measurement_concept_id
            ...
            , data_partner_id
            , payload
        FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: PEDSnet/Site 777/transform/03 - prepared/observation` o
