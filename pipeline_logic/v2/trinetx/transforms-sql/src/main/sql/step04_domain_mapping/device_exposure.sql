CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 777/transform/04 - domain mapping/device_exposure` AS

WITH diag_med_proc AS (
    SELECT 
        diagnosis.patient_id AS site_patient_id,
        CAST(diagnosis.date AS date) AS device_exposure_start_date,
        CAST(diagnosis.date AS timestamp) AS device_exposure_start_datetime,
    ...
    SELECT 
        ...
        CAST(data_partner_id as int) as data_partner_id,
        payload
    FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 777/transform/03 - prepared/medication` medication
