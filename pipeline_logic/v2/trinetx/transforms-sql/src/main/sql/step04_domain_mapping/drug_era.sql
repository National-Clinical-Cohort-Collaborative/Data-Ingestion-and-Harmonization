CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 777/transform/04 - domain mapping/drug_era`
...
(
	SELECT DISTINCT
		...
		c.concept_id
	FROM `/UNITE/OMOP Vocabularies/concept_ancestor` ca
		INNER JOIN `/UNITE/OMOP Vocabularies/concept` c ON ca.ancestor_concept_id = c.concept_id
...
(-- Normalize DRUG_EXPOSURE_END_DATE to either the existing drug exposure end date, or add days supply, or add 1 day to the start date
	SELECT
		...
		, data_partner_id
		, payload
	FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 777/transform/04 - domain mapping/drug_exposure` d
