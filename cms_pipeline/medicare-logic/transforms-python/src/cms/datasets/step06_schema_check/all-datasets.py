...
def make_transform(domain, pkey):
    ...
    # else:
    input_dataset = configs.transform + "05 - safe/" + domain
    ...
    @transform_df(
        ...
        my_input=Input(input_dataset, checks=all_checks),
    )
    def compute_function(my_input):
        # dropping extra care_site_id column for original ids that were necessary for care_site mapping table
        # meant only for a few select researchers, should not remain in the pipeline after step 5
        if domain == "care_site":
