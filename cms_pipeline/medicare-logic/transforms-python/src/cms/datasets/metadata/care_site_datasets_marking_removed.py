...
def make_transform(dataset, provider_col, person_col):
    input_dataset = configs.transform + "06_5 - add concept names/" + dataset
    @transform_df(
        Output("/UNITE/[PPRL] CMS Data & Repository/pipeline/logic/cms/datasets/metadata/" + dataset + "_marking_removed"),
    ...
    def compute_function(input_df):
        if dataset == "health_system":
            input_df = input_df.drop("chsp_compendium_year", "chsp_health_sys_id", "chsp_ccn", "health_system_hash_id")
