...
@transform_df(
    Output("/UNITE/[PPRL] CMS Data & Repository/pipeline/metadata/person_map"),
    pprl=Input("/UNITE/[PPRL] CMS Data & Repository/pipeline/transform/02 - schema applied/pprl_mapping"),
...
def compute(pprl, institutions, person_ids):
    ...
    cms = pprl.filter(F.col("institution") == "CMS").alias("cms")
    others = pprl.filter((F.col("institution") != "CMS") & (F.col("institution") != "CMS-MEDICAID")).alias("others")
