CREATE TABLE `/UNITE/[PPRL] CMS Data & Repository/pipeline/transform/04 - domain_mapping/macro_visit` AS
    ...
    --             do this case separately for LT or SN in visit detail -- Condition 2: Merge with previous visit if start date is one day after
    ...
    --             --LEAD() next row,  
    ---             Condition 3: Merge with next visit if end date is inside the existing visit
    --             WHEN visit_end_date >= LEAD(visit_end_date) OVER (PARTITION BY person_id ORDER BY visit_start_date, visit_end_date) THEN 0
    --             do this case separately for LT or SN in visit detail-- if difference is greater than one day
    ...
    with cte_merge_visit as (
     SELECT distinct 
        ...
        WHERE v.visit_end_date is not null and v.visit_start_date is not null 
