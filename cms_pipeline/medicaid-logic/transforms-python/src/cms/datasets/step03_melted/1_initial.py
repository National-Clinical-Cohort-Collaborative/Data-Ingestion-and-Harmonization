...
def make_transform(dataset, groupName, group):
    ...
    def compute_function(input_df, ahrq_codes):
        if "column_group" in group:
            ...
            columnNames = [group["column_group"]["column"] + "_" + str(i) for i in span]
            ...
            if dataset.dates["type"] == "static":
                ...
                startDateKeyColumns = [F.lit(dataset.dates["start"] + str(i)).cast("string").alias("start_date_key") for i in span]
                startDateValueColumns = [F.col(dataset.dates["start"] + str(i)).cast("string").alias("start_date_value") for i in span]
